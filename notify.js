...
=================微信server酱=================
*/
//此处填你申请的SCKEY.(环境变量名 PUSH_KEY)
let SCKEY = '';
/*
==================Bark App===================
*/
//此处填你的Bark App信息.(设备码xxx 例如: https://api.day.app/xxx)
let BARK_PUSH = '';
//Bark App推送铃声(在Bark App查看铃声列表填写)
let BARK_SOUND = '';
/*
=================Telegram Bot=================
*/
//此处填你的Telegram Bot Token,是必填项.例如：1077xxx4424:AAFjv0FcqxxxxxxgEMGfi22B4yh15R5uw (环境变量名 TG_BOT_TOKEN)
let TG_BOT_TOKEN = '';
//此处填你的Telegram用户id,是必填项.例如：129xxx206 (环境变量名 TG_USER_ID)
let TG_USER_ID = '';
//tg推送http代理设置,非必填.
//例如:127.0.0.1(环境变量名 TG_PROXY_HOST)
let TG_PROXY_HOST = '';
//例如:1080(环境变量名 TG_PROXY_PORT)
let TG_PROXY_PORT = '';
//tg代理配置认证参数
let TG_PROXY_AUTH = '';
...
let DD_BOT_TOKEN = '';
//钉钉Bot密钥(机器人安全设置页面,加签一栏下面显示的SEC开头的字符串)
let DD_BOT_SECRET = '';

/*
===============企业微信机器人=================
*/
//企业微信机器人的 webhook(详见文档 https://work.weixin.qq.com/api/doc/90000/90136/91770)  例如：693a91f6-7xxx-4bc4-97a0-0ec2sifa5aaa (环境变量名 QYWX_KEY)
let QYWX_KEY = '';
...
*/
/*
企业微信应用消息的值(详见文档 https://work.weixin.qq.com/api/doc/90000/90135/90236)
(环境变量名 QYWX_AM)
依次填入corpid,corpsecret,touser(多个成员ID使用|隔开),agentid,消息类型(选填,不填默认文本消息类型) 注意用,号隔开(英文输入法的逗号)
例如：wwcff56746d9adwers,B-791548lnzXBE6_BWfxdf3kSTMJr9vFEPKAbh6WERQ,mingcheng,1000001,2COXgjH2UIfERF2zxrtUOKgQ9XklUqMdGSWLBoW_lSDAdafat
可选推送消息类型(推荐使用图文消息（mpnews）):
- 文本卡片消息: 0 (数字零)
- 文本消息: 1 (数字一)
- 图文消息（mpnews）: 素材库图片id, 可查看此教程(http://note.youdao.com/s/HMiudGkb)或(https://note.youdao.com/ynoteshare1/index.html?id=1a0c8aff284ad28cbd011b29b3ad0191&type=note)
*/
let QYWX_AM = '';
/*
==================iGot聚合====================
*/
//iGot的推送key xxx.例如：https://push.hellyw.com/xxx
let IGOT_PUSH_KEY = '';
/*
====================Push +====================
*/
...
let PUSH_PLUS_TOKEN = '*****4a2c73e47d78a8475500e2*****';
...
async function sendNotify(text, desp, params = {}, author = '') {
    ...
    await Promise.all([
        ...
        pushPlusNotify(text, desp)//Push Plus
    ])
    //由于上述两种微信通知需点进去才能查看到详情，故text(标题内容)携带了账号序号以及昵称信息
...
function CoolPush(text, desp) {
    return new Promise(resolve => {
        if (QQ_SKEY) {
            ...
            // 已知敏感词
            text = text.replace(/京豆/g, "Bean");
            desp = desp.replace(/京豆/g, "Bean");
            desp = desp.replace(/🐶/g, "");
            desp = desp.replace(/红包/g, "Money");
                ...
                try {
                    if (err) {
                        ...
                        if (data.code === 200) {
                            ...
                            console.log(`QQ酷推出现错误，${data.message}：${data.data}\n`)
                        } else {
                            console.log(`QQ酷推推送异常: ${JSON.stringify(data)}`);
                        }
                    }
                } catch (e) {
                    $.logErr(e, resp);
